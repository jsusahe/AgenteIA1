...
def clean_html(text):
    """Elimina etiquetas HTML y contenido no deseado."""
    if not text:
        return text
    # Eliminar etiquetas HTML
    text = re.sub(r'<[^>]+>', '', text)
    # Eliminar URLs de imágenes
    text = re.sub(r'http\S+\.(jpg|jpeg|png|gif|webp|svg|bmp)\S*', '', text, flags=re.IGNORECASE)
    # Eliminar texto "www." y "https://" que pueda quedar
    text = re.sub(r'(https?://)?(www\.)?\S+\.(jpg|jpeg|png|gif|webp|svg|bmp)', '', text, flags=re.IGNORECASE)
...
def generate_html_document(summary_data, audio_filename, output_filename="index.html"):
    ...
    Incluye la sección de histórico SIEMPRE.
    """
        ...
        for f in os.listdir(history_folder):
            ...
            if f.startswith("resumen_") and f.endswith(".html"):
                try:
                    ...
                    # Solo incluir archivos con fecha válida y que no sean el día actual
    ...
    # --- Limpiar el resumen general ---
    summary_text = summary_data.get('summary', 'No se pudo generar el resumen.')
    ...
    <head>
        ...
        <style>
            ...
            iframe, video {{
                display: none; /* Ocultar iframes y videos */
            }}
        </style>
    </head>
    <body>
    ...
    for item in summary_data.get('news', []):
        ...
        # Limpiar el resumen de la noticia
        resumen = clean_html(item.get('resumen', 'Sin resumen disponible.'))
        fuente = item.get('fuente', 'Fuente desconocida')
        url = item.get('url', '#')
        tipo = item.get('tipo', 'externo')
        
        # Si la fuente es genérica, intentar mejorarla
        if fuente == "Fuente externa" and tipo == 'youtube':
            fuente = "YouTube"
        elif fuente == "Fuente externa":
...
if __name__ == '__main__':
    test_data = {
        ...
        "news": [
            ...
            },
            {
                ...
                "fuente": "TechCrunch",
                "url": "https://techcrunch.com/ejemplo"
            }
        ]
    }
    ...
    if not os.path.exists("historial"):
        ...
        with open("historial/resumen_2026-07-31.html", "w") as f:
            f.write("<h1>Prueba histórica 2</h1>")
