...
def authenticate_drive():
    ...
    Si no hay credenciales o han expirado, inicia el flujo OAuth.
    """
    creds = None
    ...
    if not creds or not creds.valid:
        ...
        # Si aún no hay credenciales, iniciar nuevo flujo
        if not creds:
        ...
        try:
            ...
            print(f"⚠️ Error al guardar token.json: {e}")
    
    # 3. Construir el servicio de Drive
    try:
        service = build('drive', 'v3', credentials=creds)
        print("✅ Servicio de Google Drive construido correctamente")
        return service
    except Exception as e:
