...
MAX_HISTORY_DAYS = 10
TOP_NEWS_LIMIT = 10  # Número de noticias externas a seleccionar
MAX_NEWS_TOTAL = 10   # Total de noticias en el resumen
# --------------------
...
def run_agent():
    ...
    # Tomar solo la primera noticia de cada canal (ya está agrupado en summary_generator)
    # Pero aquí limitamos a 3 noticias de YouTube
    youtube_limited = youtube_videos[:3]  # Solo 3 noticias de YouTube
    
    for video in youtube_limited:
        combined_news.append({
            'title': video.get('title', 'Sin título'),
            'resumen': video.get('transcript', '')[:200] if video.get('transcript') else 'Transcripción no disponible',
            'fuente': f"YouTube: {video.get('channel', 'Desconocido')}",
            'url': video.get('url', '#'),
            'tipo': 'youtube'
        })
    
    # Añadir noticias externas (completar hasta MAX_NEWS_TOTAL)
    for article in external_news[:MAX_NEWS_TOTAL - len(youtube_limited)]:
        combined_news.append({
            'title': article.get('title', 'Sin título'),
            'resumen': article.get('summary', '')[:200],
    ...
    # --- PASO 9: DESCARGAR HISTÓRICOS DESDE DRIVE Y ACTUALIZAR index.html ---
    print("\n🔄 PASO 9: Sincronizando históricos desde Google Drive...")
    try:
        download_history_from_drive(HISTORY_FOLDER)
    ...
    print(f"📰 Fuentes: YouTube (3) + {len(external_news)} externas.")
    print("="*60)
