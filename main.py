...
def download_history_from_drive(history_folder="historial"):
    """
    Descarga todos los archivos históricos (HTML y MP3) desde Drive.
    """
    ...
    # Buscar archivos HTML y MP3 en Drive
...
def run_agent():
    ...
    # --- PASO 0: DESCARGAR HISTÓRICOS (HTML Y MP3) DESDE DRIVE ---
    print("\n🔄 PASO 0: Descargando históricos desde Google Drive...")
    try:
        service = authenticate_drive()
        if service:
            # Buscar archivos HTML y MP3 en Drive
            ...
            if files:
                ...
                for file in files:
                    filename = file['name']
                    if not filename.startswith('resumen_'):
                        continue
                    filepath = os.path.join(HISTORY_FOLDER, filename)
                    ...
                    print(f"✅ Descargado: {filename}")
                    downloaded_count += 1
                print(f"✅ {downloaded_count} archivos descargados desde Drive.")
            else:
                print("ℹ️ No hay archivos históricos en Drive.")
    except Exception as e:
        print(f"⚠️ Error al descargar históricos: {e}")
    ...
    try:
        ...
        # COPIAR EL AUDIO MÁS RECIENTE A LA RAÍZ
        if audio_file and os.path.exists(os.path.join(HISTORY_FOLDER, audio_file)):
            ...
            print(f"✅ resumen_ia.mp3 actualizado en la raíz.")
        else:
            print("⚠️ No se encontró audio para copiar a la raíz.")
    except Exception as e:
