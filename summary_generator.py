...
def select_representative_news(news_items):
    ...
    # Ordenar por fecha (más reciente primero) si existe
...
def generate_summary_fallback(selected_news):
    """Genera un resumen básico cuando DeepSeek falla."""
...
def generate_summary(news_items):
    ...
    # --- PROMPT MEJORADO PARA RESUMEN MÁS LARGO Y FUENTES CLARAS ---
    system_prompt = """
    Eres un asistente que genera resúmenes de noticias de IA. **DEBES** devolver SOLO un JSON válido con la siguiente estructura EXACTA. No añadas texto antes, después, ni explicaciones.

    {
      "summary": "Resumen general en español de 4-5 párrafos (mínimo 500 palabras) que mencione TODAS las noticias listadas. Desarrolla cada idea con detalles y contexto.",
      "news": [
        {
          "title": "Título de la noticia (TRADUCIDO al español)",
          "resumen": "Resumen extenso en español (mínimo 3 renglones, 150-200 caracteres)",
          "fuente": "Nombre de la fuente (ej. YouTube: EDteam, TechCrunch, MIT Technology Review)",
    ...
    **REGLAS OBLIGATORIAS:**
    ...
    2. El campo 'summary' DEBE ser extenso (4-5 párrafos, mínimo 500 palabras).
    3. Traduce TÍTULOS y RESÚMENES al español.
    4. Cada 'resumen' debe tener al menos 3 renglones (150-200 caracteres).
    5. El campo 'fuente' DEBE identificar claramente la fuente (ej. "YouTube: EDteam", no solo "YouTube").
    ...
    user_prompt = f"Noticias del día:\n\n{news_info_text}\n\nGenera el JSON con el resumen general (4-5 párrafos) y la lista de noticias."
    ...
    payload = {
        ...
        "temperature": 0.7,  # Un poco más de creatividad para resúmenes más largos
        "max_tokens": 5000   # Suficiente espacio para un resumen extenso
    }
    ...
    for attempt in range(2):
        try:
            ...
            # Verificar que los resúmenes de noticias tengan al menos 150 caracteres
