...
def generate_summary(news_items):
    ...
    **INSTRUCCIONES OBLIGATORIAS:**
    ...
    3. Cada noticia DEBE tener un resumen en español de al menos 3 renglones (150-200 caracteres).
    4. El 'summary' DEBE mencionar explícitamente cada una de las noticias.
    5. Tu respuesta DEBE ser SOLO el JSON. No añadas texto antes, después o explicaciones.
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "content": user_prompt + "\n\nDEVUELVE SOLO EL JSON. LOS TÍTULOS DEBEN ESTAR TRADUCIDOS AL ESPAÑOL."}
        ],
        ...
        if len(summary_data.get('news', [])) < MIN_NEWS_TOTAL:
            ...
            for item in selected_news:
                if item.get('title') not in existing_titles:
                    # Forzar traducción del título si está en inglés
                    titulo_traducido = item.get('title', 'Sin título')
                    # Si el título está en inglés, añadir una nota (DeepSeek debería traducirlo)
                    summary_data['news'].append({
                        'title': titulo_traducido,
                        'resumen': item.get('resumen', 'Noticia destacada')[:300],
                        'fuente': item.get('fuente', 'Fuente desconocida'),
                        'tipo': item.get('tipo', 'externo'),
                        'url': item.get('url', '#')
                    })
                    print(f"✅ Añadida noticia faltante: {titulo_traducido}")
