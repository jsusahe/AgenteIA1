...
def select_representative_news(news_items):
    ...
    # --- NUEVO: Seleccionar hasta 3 noticias de YouTube (las más recientes) ---
    # Ordenar las noticias de YouTube por fecha (más reciente primero)
    youtube_news.sort(key=lambda x: x.get('published_at', ''), reverse=True)
    
    selected_news = []
    # Tomar hasta 3 noticias de YouTube (o todas las que haya)
    ...
    # Completar con noticias externas hasta alcanzar MIN_NEWS_TOTAL
    remaining_slots = MIN_NEWS_TOTAL - len(selected_news)
    if remaining_slots > 0 and external_news:
        # Ordenar externas por relevancia (usar las primeras)
    ...
    # Si aún faltan noticias, duplicar las que ya tenemos
