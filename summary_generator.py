...
def extract_json_from_response(text):
    ...
    # Buscar desde la primera llave hasta la última llave que cierre
    start = text.find('{')
    if start != -1:
        # Buscar la última llave de cierre que tenga un '}' después de un '}'
        depth = 0
        end = -1
        ...
        if end != -1:
            ...
            try:
                ...
                json_str = re.sub(r'(?<!\\)\'', '"', json_str)  # Comillas simples a dobles
                json_str = re.sub(r'([{,])\s*(\w+)\s*:', r'\1"\2":', json_str)  # Claves sin comillas
                try:
                    return json.loads(json_str)
                except:
                    pass

    # ESTRATEGIA 4: Si hay texto, intentar construir un JSON básico
    if text:
        # Intentar extraer un resumen general
        lines = text.split('\n')
        summary_lines = []
        for line in lines:
            line = line.strip()
...
def generate_summary(news_items):
    ...
    # --- Prompt mejorado para forzar respuesta JSON ---
    system_prompt = """
    ...
    **INSTRUCCIÓN OBLIGATORIA:** Tu respuesta DEBE ser SOLO el JSON. No añadas texto antes, después o explicaciones. NO uses comillas simples.
    """
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "content": user_prompt + "\n\nDEVUELVE SOLO EL JSON. NO AÑADAS TEXTO ADICIONAL."}
        ],
    ...
    try:
        ...
        if summary_data is None:
            ...
            try:
                # Buscar todo lo que parezca un JSON
