...
MAX_NEWS_TOTAL = 10  # Número máximo de noticias en el resumen final
# ---------------------

def select_representative_news(news_items):
    """
    Selecciona noticias garantizando al menos una por canal de YouTube,
    y completa con las más relevantes de fuentes externas.
    """
    if not news_items:
        return []

    # Separar noticias de YouTube y externas
    ...
    for item in youtube_news:
        # Extraer el nombre del canal (ej. "YouTube: EDteam" -> "EDteam")
    ...
    for canal, items in channels.items():
        if items:
            # Elegir la noticia más reciente o con más contenido (por simplicidad, la primera)
            selected_news.append(items[0])
            print(f"📌 Seleccionada noticia de {canal}: {items[0].get('title', 'Sin título')}")

    # Completar con noticias externas hasta alcanzar MAX_NEWS_TOTAL
    remaining_slots = MAX_NEWS_TOTAL - len(selected_news)
    if remaining_slots > 0 and external_news:
        # Ordenar noticias externas por relevancia (usando un criterio simple)
        # Podríamos ordenar por fecha, pero por ahora usamos las primeras
...
def generate_summary(news_items):
    ...
    # --- PASO 2: Construir el prompt para DeepSeek ---
    news_info_text = ""
    ...
    # --- PROMPT MEJORADO PARA FORZAR EL ESPAÑOL ---
    system_prompt = """
