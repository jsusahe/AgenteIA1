...
# Archivo de caché para IDs de canales
CHANNEL_CACHE_FILE = "channel_cache.json"

def load_channel_cache():
    """Carga el caché de IDs de canales desde un archivo JSON."""
...
def save_channel_cache(cache):
    """Guarda el caché de IDs de canales en un archivo JSON."""
    try:
        with open(CHANNEL_CACHE_FILE, 'w', encoding='utf-8') as f:
            json.dump(cache, f, indent=2, ensure_ascii=False)
    except Exception as e:
        print(f"⚠️ Error al guardar caché de canales: {e}")
...
def get_channel_id(url):
    ...
    if url in cache:
        print(f"📦 ID del canal obtenido del caché: {cache[url]}")
        return cache[url]
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 429:
                ...
                print(f"⚠️ Límite de cuota de YouTube API alcanzado. Esperando {wait_time} segundos...")
                time.sleep(wait_time)
                continue
            elif response.status_code == 403:
                print(f"⚠️ Acceso denegado a la API de YouTube. Verifica tu clave API para {username}.")
                return None
            elif response.status_code == 400:
                print(f"⚠️ Solicitud incorrecta a la API de YouTube. Verifica el nombre del canal: {username}.")
                return None
...
def get_videos_from_channel(channel_id):
    ...
    params = {
        ...
        'maxResults': 5,  # Reducido para ahorrar cuota
        'key': YOUTUBE_API_KEY
    }
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 429:
                ...
                print(f"⚠️ Acceso denegado a la API de YouTube. Verifica tu clave API para el canal {channel_id}.")
                return []
            elif response.status_code == 400:
                print(f"⚠️ Solicitud incorrecta a la API de YouTube para el canal {channel_id}.")
                return []
...
def get_transcript(video_id):
    """
    Obtiene la transcripción de un video usando yt-dlp con subtítulos forzados.
    """
    ydl_opts = {
        'quiet': True,
        'skip_download': True,
...
def fetch_all_youtube_content():
    ...
    for channel_url in CHANNEL_URLS:
        ...
        if not channel_id:
            ...
            # Pequeña pausa incluso si falla
            time.sleep(1)
            continue
        ...
        for video in videos:
            ...
            if transcript:
                ...
                # Fallback a descripción si no hay subtítulos
                if video.get('description'):
                    video['transcript'] = video['description'][:500]
                    all_videos_with_transcripts.append(video)
                    print(f"  ✅ Usando descripción como fallback ({len(video['description'][:500])} caracteres)")
                else:
                    print(f"  ❌ No se pudo obtener transcripción ni descripción")
